  ...
  return (
    <section className="py-12 sm:py-16 lg:py-20 px-4 bg-gradient-to-b from-muted/50 to-background overflow-hidden">
      <div className="container mx-auto">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-muted-foreground text-base sm:text-lg max-w-2xl mx-auto">
            Nossa loja fica no Umbará — venha nos visitar e conheça as melhores armações e lentes pessoalmente!
          </p>
        </div>
      ...
      {lightboxOpen && (
        ...
        >
          <button onClick={closeLightbox} className="absolute top-4 right-4 text-white hover:text-primary p-2 transition-colors z-50" aria-label="Fechar">
            <X className="w-8 h-8" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); prevImage(); }} className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 text-white hover:text-primary p-2 sm:p-3 bg-white/10 rounded-full transition-all hover:scale-110" aria-label="Imagem anterior">
            <ChevronLeft className="w-6 h-6 sm:w-8 sm:h-8" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); nextImage(); }} className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 text-white hover:text-primary p-2 sm:p-3 bg-white/10 rounded-full transition-all hover:scale-110" aria-label="Próxima imagem">
            <ChevronRight className="w-6 h-6 sm:w-8 sm:h-8" />
          </button>
          <div className="max-w-5xl max-h-[85vh] w-full" onClick={(e) => e.stopPropagation()}>
            <img src={galleryImages[currentImage].src} alt={galleryImages[currentImage].alt} className="w-full h-full object-contain rounded-lg" />
